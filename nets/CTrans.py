...
#         self.qkv = nn.Linear(dim, dim * 3, bias=qkv_bias)  # qkv 是将输入特征映射为查询、键、值向量的线性层
...
#         # B_ 是一个变量名，用于表示批次大小（batch size）。使用 _ 作为后缀通常是为了避免与 Python 中的关键字或其他变量名冲突
#         # 维度的顺序变为 (3, B_, num_heads, N, C // num_heads)

#         q, k, v = qkv[0], qkv[1], qkv[2]

#         q = q * self.scale  # 对查询张量 q 进行缩放操作
...
#             act_layer=nn.GELU):
#         super().__init__()
#         out_features = out_features or in_features  # 如果 out_features 为 None，则将其值设置为输入特征的维度 in_features
#         hidden_features = hidden_features or in_features
#         self.fc1 = nn.Linear(in_features, hidden_features)  # 将输入特征的维度 in_features 转换为隐藏层特征的维度 hidden_features
#         self.act = act_layer()
...
#         shortcut = self.cpe[0](x, size)  # 对输入张量 x 进行卷积位置编码
#         x = self.norm1(shortcut)
#         x = x.view(B, H, W, C)

#         pad_l = pad_t = 0  # 计算填充的左右和上下边界，以使 x 的宽度和高度能够被 self.window_size 整除s
#         pad_r = (self.window_size - W % self.window_size) % self.window_size  #0
#         pad_b = (self.window_size - H % self.window_size) % self.window_size  #0

#         # 使用 F.pad 函数对 x 进行填充，以使其宽度和高度能够被 self.window_size 整除
#         x = F.pad(x, (0, 0, pad_l, pad_r, pad_t, pad_b))
#         _, Hp, Wp, _ = x.shape  # 获取填充后 x 的新形状信息，并将其分别赋值给 Hp 和 Wp。

#         # 将填充后的 x 分割为窗口，并将其形状变换为 (B * num_windows, window_size * window_size, C)
...
#         # 如果存在右侧或底部的填充，则通过切片操作将 x 裁剪为原始大小
