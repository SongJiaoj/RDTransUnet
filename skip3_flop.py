...
#from .CTrans3 import ChannelTransformer
...
class Block_ViT(nn.Module):
    def __init__(self, config, vis, channel_num):
        ...
        self.attn_norm =  LayerNorm(448,eps=1e-6)
...
class ChannelTransformer(nn.Module):
    def __init__(self, config, vis, img_size, channel_num=[64, 128, 256], patchSize=[32, 16, 8]):
        super().__init__()
...
class UpBlock_attention(nn.Module):
    ...
    def forward(self, x, skip_x):
        # print('111',x.shape)
        # print('112',skip_x.shape)
        up = self.up(x)
        # print('113',up.shape)
        up=self.eca(up)
        # print('114',up.shape)
        skin_x=self.eca(skip_x)
        # print('115',skin_x.shape)
...
class skip3(nn.Module):
    def __init__(self, config,n_channels=3, n_classes=1,img_size=560,vis=False):
        ...
        self.ss = nn.Conv2d(in_channels*8, in_channels*4, kernel_size=(1,1), stride=(1,1))
